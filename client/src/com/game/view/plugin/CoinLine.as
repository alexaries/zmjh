package com.game.view.plugin
{
	 ...
	 * 直线型金币排布
	 */	
	public class CoinLine extends Component
	{
		private var _coinLineArr:Array = [];
		public var coinValue:int;
		
		public function CoinLine(item:XML, titleTxAtlas:TextureAtlas)
		{
		...
		public function hitTest(player:PlayerRun):int{
			//人物的坐标为底部中点
			var _score:int=0;
			for each(var coin:MovieClip in _coinLineArr){
				if(Math.abs(this.panel.x + coin.x + coin.width * .5 - player.x) < (coin.width * .5 + player.width * .5 ) 
					...
					coin.alpha=0;
					_coinLineArr.splice(_coinLineArr.indexOf(coin),1);
					_score+=coinValue;
		...
		{
			return new CoinLine(_configXML, _titleTxAtlas);
		}
