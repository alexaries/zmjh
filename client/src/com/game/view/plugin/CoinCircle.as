package com.game.view.plugin
{
	...
	public class CoinCircle extends Component
	{
		public function CoinCircle(item:XML, titleTxAtlas:TextureAtlas)
		{
		...
		private var _coinCircleArr:Array = [];
		...
		public function hitTest(player:PlayerRun):int{
			//人物的坐标为底部中点
			var _score:int=0;
			for each(var coin:MovieClip in _coinCircleArr){
				if(Math.abs(this.panel.x + coin.x + coin.width * .5 - player.x) < (coin.width * .5 + player.width * .5 ) 
					...
					coin.alpha=0;
					_coinCircleArr.splice(_coinCircleArr.indexOf(coin),1);
					_score+=coinValue;
		...
		{
			return new CoinCircle(_configXML, _titleTxAtlas);
		}
