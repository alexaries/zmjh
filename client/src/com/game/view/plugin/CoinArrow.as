package com.game.view.plugin
{
	...
	public class CoinArrow extends Component
	{
		public function CoinArrow(item:XML, titleTxAtlas:TextureAtlas)
		{
		...
		private var _coinArrowArr:Array = [];
		...
		public function hitTest(player:PlayerRun):int{
			//人物的坐标为底部中点
			var _score:int=0;
			for each(var coin:MovieClip in _coinArrowArr){
				if(Math.abs(this.panel.x + coin.x + coin.width * .5 - player.x) < (coin.width * .5 + player.width * .5 ) 
					...
					coin.alpha=0;
					_coinArrowArr.splice(_coinArrowArr.indexOf(coin),1);
					_score+=coinValue;
		...
		{
			return new CoinArrow(_configXML, _titleTxAtlas);
		}
