package com.game.view.plugin
{
	...
	public class PlayerRun extends Entity
	{
		...
		public function get gameplayTime():int{
		 ...
		 * 玩家死掉的时间
		 */		
		private var _deadTime:Number = 0;
		...
		public var fallY:Number;
		public function PlayerRun(view:View, x:Number = 0, y:Number = 0, type:String = 'player')
		{
			_view = view;
			_roleLibrary = {};
		...
		{
			if(jumpCount>1 || _bomState ) return;
			_jumpState = true;
			_totalTime = 0;
		...
		private function setMoveY(num:Number) : void
		{
			...
			//if(_totalTime < t * 2 - .1)
			var y:int=LayerManager.instance.height - 40;
			if(_moveSpeedY>0 || this.y <= y)
			{
				this.y -= _moveSpeedY;
				positionLimitY();
			}
		 ...
		 * 
		 */		
		private function setMoveDie(num:Number) : void
		{
			_deadTime += num;
			_moveSpeedY = _moveSpeedYState - GRAVITY * _deadTime;
			//if(_totalTime < t * 2 - .1)
		...
		{
			if(this.y > LayerManager.instance.height - 40)
			{
