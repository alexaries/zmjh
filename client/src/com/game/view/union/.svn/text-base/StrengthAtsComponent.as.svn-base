package com.game.view.union
{
	...
	public class StrengthAtsComponent extends Component
	{
		public function StrengthAtsComponent(item:XML, titleTxAtlas:TextureAtlas)
		{
		...
		{
			if(_info.ats >= _view.strength.strengthData.length)
				removeTouchable(_martialBtn);
			else
				addTouchable(_martialBtn);
			/*if(player.fight_soul < _strengthData.soul || player.strength_exp < _strengthData.exp_add)
				removeTouchable(_martialBtn);
			else
				addTouchable(_martialBtn);*/
		}
		...
		private function renderTF():void
		{
			...
			if(_info.ats >= _view.strength.strengthData.length)
			{
				_strengthData = _view.strength.strengthData[_info.ats - 1];
				_levelTF_1.text = _info.ats.toString();
				_levelTF_2.text = "无";
				_upgradeTF_1.text = _strengthData.ads.toString();
				_upgradeTF_2.text = "无";
				_expTF.text = _strengthData.exp_all + "/" + _strengthData.exp_all;
				_expBar.width = 104;
				_info.ats_exp = DataList.list[0];
				return;
			}
			
			_strengthData = _view.strength.strengthData[_info.ats];

			_levelTF_1.text = _info.ats.toString();
			_levelTF_2.text = (_info.ats + 1).toString();
			_upgradeTF_1.text = (_info.ats == 0?"0":_view.strength.strengthData[_info.ats - 1].ads.toString());
			_upgradeTF_2.text = _strengthData.ads.toString();
			
			_expTF.text = _info.ats_exp + "/" + _strengthData.exp_all;
			
			_expBar.width = _info.ats_exp / _strengthData.exp_all * 104;
		}
		...
		private function upgradeStrength() : void
		{
			...
			_info.ats_exp += _strengthData.exp_add;
		...
		private function tweenEffect() : void
		{
			...
			_view.strength.panel.touchable = false;
			if(_info.ats_exp >= _strengthData.exp_all)
			{
				_tweenEffect.setStart(_expBar, "width", 104, .5, 
					function () : void
					{
						...
						_effectShow.start();
						if(_info.ats == (_view.strength.strengthData.length - 1))
						{
							tweenComplete();
						}
						else
						{
							_expBar.width = 0;
							_expTF.text = (_info.ats_exp - _strengthData.exp_all) + "/" + _view.strength.strengthData[_info.ats + 1].exp_all;
							_tweenEffect.setStart(_expBar, "width", (_info.ats_exp - _strengthData.exp_all) / _view.strength.strengthData[_info.ats + 1].exp_all * 104, .25, tweenComplete);
						}
					}
				);
			}
			...
			{
				_tweenEffect.setStart(_expBar, "width", _info.ats_exp / _strengthData.exp_all * 104, .5, tweenComplete);
			}
		}
		
		private function tweenComplete() : void
		{
			_view.strength.panel.touchable = true;
			this.panel.touchable = true;
			if(_info.ats_exp >= _strengthData.exp_all)
			{
				showInfo();
				_info.ats_exp -= _strengthData.exp_all;
				_info.ats++;
		...
		private function showInfo() : void
		{
			...
			info += ("的内功等级从" + _info.ats + "级提升到" + (_info.ats + 1) + "级");
		...
		{
			return new StrengthAtsComponent(_configXML, _titleTxAtlas);
		}
