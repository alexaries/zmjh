package com.game.view.map.player
{
	...
	import com.game.view.map.TDprop;
	...
	public class TDPlayerEntity extends Entity
	{
		...
		// 移动时间
		public static const STEP_TIME:Number = 2;		
		// 人物y轴偏移
		...
		private const TowerDefenceHpMax:int=500;
		
		
		private var _towerDefenceHp:int;
		
		public function get towerDefenceHp():int
		{
			return _towerDefenceHp;
		}

		public function set towerDefenceHp(value:int):void
		{
			_towerDefenceHp = value;
		}

		public function getHurt(damage:int):void{
			_towerDefenceHp-=damage;
			if(_towerDefenceHp>0){
				_hpImage.width=(_towerDefenceHp/TowerDefenceHpMax)*_hpImageWidthMax;
			}
			else{
				_hpImage.width=0;
			}
		...
		public function TDPlayerEntity(x:Number = 0, y:Number = 0, type:String = 'player')
		{
			_view = View.instance;			
			_roleLibrary = {};
			super(x, y, type);
		...
		private var _hpImageWidthMax:int;
		private var _hpImage:Image;
		private function initHpBar():void{
			var _hp:Shape = new Shape;
			_hp.graphics.clear();  
			_hp.graphics.beginFill(0xFF0000);
			_hp.graphics.drawRect(0,0,100,10);
			_hp.graphics.endFill();
			_hp.cacheAsBitmap = true;
			var _bmpData:BitmapData = new BitmapData(100, 200,true,0);
			_bmpData.draw(_hp);
			var _Texture:Texture = Texture.fromBitmapData(_bmpData);
			_hpImage = new Image(_Texture);
			_hpImage.x=-50;
			_hpImage.y=-160;
			this.addChild(_hpImage);
			_hpImageWidthMax=_hpImage.width;
			
			_towerDefenceHp=TowerDefenceHpMax;
		}
		...
		private function get textureAtlas() : TextureAtlas
		{
			if (!_sTextureAtlas)
			{
				//var bitmapData:Bitmap = _view.res.getAssetsObject(V.PUBLIC, V.PLAYER, GameConfig.PLAYER_RES, "Textures") as Bitmap;
		...
		private var _roads:Vector.<TDprop>;
		...
		public function setPath(info:SingleLineInfo, roads:Vector.<TDprop>, useFreeDice:Boolean = false) : void
		{
		...
		private function checkFreeMove() : void
		{
			...
			else 
			{
				...
				else 
				{
					...
					{
						_curNode = new Node();
						_curNode.curIndex =  new Point((view as TowerDefenceView).mapInfo.playerPosition.gx, (view as TowerDefenceView).mapInfo.playerPosition.gy);
					}
					_curRole.alpha = 0.8;
		...
		private function onComplete(isBlock:Boolean = false) : void
		{
			if (_curSingleLineInfo.roadPoints.length == 0)
			{
				...
				if (view)
				{
					(view as TowerDefenceView).mapInfo.playerPosition.gx = _curNode.curIndex.x;
					(view as TowerDefenceView).mapInfo.playerPosition.gy = _curNode.curIndex.y;
				}
		...
		private function setMove() : void
		{
			...
			setDirection((view as TowerDefenceView).mapInfo.playerPosition.gx, (view as TowerDefenceView).mapInfo.playerPosition.gy);
			
			if(!_useFreeDice)
			{
				_tween = new Tween(this, step_time);
				_tween.moveTo(
					_curNode.curIndex.x * (view as TowerDefenceView).mapInfo.titleWidth + (view as TowerDefenceView).mapInfo.titleWidth/2 + (view as TowerDefenceView).mapInfo.mpx,
					_curNode.curIndex.y * (view as TowerDefenceView).mapInfo.titleHeight + (view as TowerDefenceView).mapInfo.titleHeight/2 + (view as TowerDefenceView).mapInfo.mpy
				);
			...
			{
				jumpStart(_curNode.curIndex.x * (view as TowerDefenceView).mapInfo.titleWidth + (view as TowerDefenceView).mapInfo.titleWidth/2 + (view as TowerDefenceView).mapInfo.mpx, 
					_curNode.curIndex.y * (view as TowerDefenceView).mapInfo.titleHeight + (view as TowerDefenceView).mapInfo.titleHeight/2 + (view as TowerDefenceView).mapInfo.mpy);
			}
		}
		...
		private function jumpStart(xPos:Number, yPos:Number, callbackFun:Function = null) : void
		{
			...
			(view as TowerDefenceView).mapLayers.mapLayer.addChild(_playerShadow);
