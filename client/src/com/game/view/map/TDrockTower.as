package com.game.view.map
{
	...
	public class TDrockTower extends Sprite
	{
		/**
		 * 文件名 
		 */		
		 ...
		 *射击频率
		 */		
		private const frequency:Number =1.2;
		...
		public function TDrockTower(view:TowerDefenceView)
		{
			_view = view;
...
/*			this.x = ix * _view.mapInfo.titleWidth ;
		...
		private function initImage() : void
		{
			...
			_circle.graphics.drawCircle(150, 150, radius);
			_circle.graphics.endFill(); 
			_circle.cacheAsBitmap = true;
			var _bmpData:BitmapData = new BitmapData(300, 300, true, 0);
		...
		private function shootBullet(e:TimerEvent):void{
			...
			Starling.juggler.delayCall(removeBullet,0.5,_bullet);
			_view.playerRole.getHurt(5);
			TweenMax.to(_bullet, 1, {bezier:[{x:Math.min(this.x,_target.x)+Math.abs(this.x -_target.x)/2, y:Math.min(this.y,_target.y)-80}, {x:_target.x, y:_target.y-50}]});
